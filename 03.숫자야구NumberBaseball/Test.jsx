...
function getNumbers() {
    ...
    const array = [];
    for (let i = 0; i < 4; i += 1) {
        const randomNumber = candidate.splice(Math.floor(Math.random() * [candidate.length - 1]), 1)[0];
        array.push(randomNumber);
    }
    return array;
}

const TestNumberBaseball = () => {
    ...
    <ul>
        {tries.map((v, i) => {
            return (
                <TestTry kry={`${i + 1}차 시도`} tryInfo={v} />
            )
        })}
    </ul>
    </>
...
export default React.memo(TestNumberBaseball);